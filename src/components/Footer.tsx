...
function StickyMobileCTABase() {
  ...
  return (
    <div
      className={`fixed bottom-5 left-4 right-4 z-40 md:hidden transition-all duration-500 ${visible ? "translate-y-0 opacity-100" : "translate-y-20 opacity-0 pointer-events-none"}`}
    >
      <div className="bg-slate-900/40 backdrop-blur-xl border border-white/20 p-2 rounded-[2rem] shadow-[0_10px_40px_rgba(0,0,0,0.2)] flex gap-2.5 w-full max-w-[24rem] mx-auto">
        <a
          href={PHONE_HREF}
          className="pointer-events-auto flex-1 h-14 rounded-full bg-gradient-to-r from-blue-600 to-blue-500 text-white font-bold flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-all"
        >
          <svg
            className="w-4 h-4"
            viewBox="0 0 24 24"
          ...
          <span className="font-semibold tracking-wide">{t("nav.call", "Llamar")}</span>
        </a>
        <a
          href={WA_HREF}
          target="_blank"
          rel="noopener noreferrer"
          className="pointer-events-auto flex-1 h-14 rounded-full bg-gradient-to-r from-[#25D366] to-[#20BD5A] text-white font-bold flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-all"
        >
